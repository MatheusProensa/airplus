package com.sulake.habbo.roomevents.wired_setup.selectors
{
   ...
   public class UsersWithHanditem extends DefaultSelectorType
   {
      private static const DEFAULT_CODES:Array = [0,2,5,7,8,9,10,27];
      
      private var _handItemDropdown:DropdownPreset;
      
      private var UnknownVarFromUsersWithHanditem_ButtonPreset_1:ButtonPreset;
      
      private var UnknownVarFromUsersWithHanditem_Vector_1:Vector.<ExpandableDropdownOption>;
      
      public function UsersWithHanditem()
      {
         super();
      }
      ...
      {
         UnknownVarFromUsersWithHanditem_Vector_1 = createOptions(DEFAULT_CODES);
         _handItemDropdown = param1.createDropdown(new DropdownParam(loc("wiredfurni.tooltip.handitem"),UnknownVarFromUsersWithHanditem_Vector_1));
         UnknownVarFromUsersWithHanditem_ButtonPreset_1 = param1.createButton(l("capture.handitem"),captureHanditem);
         var _loc4_:SectionPreset = param1.createSection(l("handitem"),param1.createSimpleListView(true,[_handItemDropdown,UnknownVarFromUsersWithHanditem_ButtonPreset_1]));
      ...
      private function ensureOptionExists(param1:int) : void
      {
         ...
         for each(var _loc2_ in UnknownVarFromUsersWithHanditem_Vector_1)
         {
            if(_loc2_.id == param1)
            {
               _loc3_ = true;
         ...
         if(!_loc3_)
         {
            UnknownVarFromUsersWithHanditem_Vector_1.push(new ExpandableDropdownOption(param1,"${handitem" + param1 + "}"));
         }
         var _loc5_:* = -1;
         _loc4_ = 0;
         while(_loc4_ < UnknownVarFromUsersWithHanditem_Vector_1.length)
         {
            if(UnknownVarFromUsersWithHanditem_Vector_1[_loc4_].id == param1)
            {
               _loc5_ = _loc4_;
            }
            _loc4_++;
         }
         _handItemDropdown.reinit(UnknownVarFromUsersWithHanditem_Vector_1,_loc5_);
      }
